...
public class ButtonBehavior<C extends ButtonBase> extends BehaviorBase<C> {
    ...
    public ButtonBehavior(C control) {
        ...
        addDefaultMapping(buttonInputMap,
            ...
            new KeyMapping(new KeyBinding(ENTER, KeyEvent.KEY_PRESSED), this::keyPressed, event -> javafx.scene.Scene.jproGetIsMacClient.apply(event)), // This is wrong, correct?
            new KeyMapping(new KeyBinding(ENTER, KeyEvent.KEY_RELEASED), this::keyReleased, event -> javafx.scene.Scene.jproGetIsMacClient.apply(event))
        );
